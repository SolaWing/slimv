...
function! SlimvSwankResponse()
    ...
    silent execute s:py_cmd . 'swank.swank_output(1)'
    let s:refresh_disabled = 0
    let s:swank_action = ''
    let s:swank_result = ''
    silent execute s:py_cmd . 'swank.swank_response("")'
...
function! SlimvCommandGetResponse( name, cmd, timeout )
    ...
    while s:swank_action == '' && localtime()-starttime < cmd_timeout
        execute s:py_cmd . "swank.swank_output( 0 )"
        silent execute s:py_cmd . 'swank.swank_response("' . a:name . '")'
    endwhile
    let s:refresh_disabled = 0
...
function! SlimvRefreshReplBuffer()
    ...
    if exists("s:input_prompt") && s:input_prompt != ''
        ...
        echo ""
        call SlimvCommand( s:py_cmd . 'swank.swank_return("' . answer . '")' )
    endif
endfunction
...
function SlimvQuitInspect( force )
    ...
    call SlimvEndUpdate()
    if a:force
        call SlimvCommand( s:py_cmd . 'swank.swank_quit_inspector()' )
    endif
...
function! SlimvConnectSwank()
    ...
    if !s:swank_connected
        ...
        execute s:py_cmd . 'swank.swank_connect("' . g:swank_host . '", ' . g:swank_port . ', "result" )'
        ...
        execute s:py_cmd . "swank.swank_require('(" . contribs . ")')"
        call SlimvSwankResponse()
        if s:SinceVersion( '2011-12-04' )
            execute s:py_cmd . "swank.swank_require('swank-repl')"
            call SlimvSwankResponse()
        endif
        if s:SinceVersion( '2008-12-23' )
            call SlimvCommandGetResponse( ':create-repl', s:py_cmd . 'swank.swank_create_repl()', g:slimv_timeout )
        endif
...
function! SlimvSend( args, echoing, output )
    ...
    if a:echoing && g:slimv_echolines != 0
        ...
        if a:output
            silent execute s:py_cmd . 'swank.append_repl("s:swank_form", 1)'
        endif
        let s:swank_form = text
    elseif a:output
        " Open a new line for the output
        silent execute s:py_cmd . " swank.append_repl('\\n', 0)"
    endif
    call SlimvCommand( s:py_cmd . 'swank.swank_input("s:swank_form")' )
    let s:swank_package = ''
    let s:swank_package_form = ''
    let s:refresh_disabled = 0
    call SlimvRefreshModeOn()
...
function! SlimvIndentUnsafe( lnum )
    ...
    if l == parent
        ...
        if func != '' && s:swank_connected
            ...
            let s:indent = ''
            silent execute s:py_cmd . 'swank.get_indent_info("' . func . '")'
...
function! SlimvSendCommand( close )
    ...
    if lastline > 0
        ...
        silent execute s:py_cmd . " swank.append_repl('Slimv error: previous EOF mark not found, re-enter last form:\\n', 0)"
    endif
endfunction
    ...
    if s:sldb_level >= 0
        ...
        if item != ''
            ...
            if search( '^Restarts:', 'bnW' ) > 0
                ...
                call SlimvQuitSldb()
                silent execute s:py_cmd . 'swank.swank_invoke_restart("' . s:sldb_level . '", "' . item . '")'
                call SlimvRefreshReplBuffer()
                return
            endif
        endif
...
function! SlimvHandleEnterInspect()
    ...
    if line[0] == '['
        if line =~ '^\[--more--\]$'
            ...
            call SlimvCommand( s:py_cmd . 'swank.swank_inspector_range()' )
            call SlimvRefreshReplBuffer()
            return
        elseif line =~ '^\[--all---\]$'
...
function! SlimvInterrupt()
    call SlimvCommand( s:py_cmd . 'swank.swank_interrupt()' )
    call SlimvRefreshReplBuffer()
endfunction
...
function! SlimvDebugCommand( name, cmd )
    if SlimvConnectSwank()
        if s:sldb_level >= 0
            ...
            call SlimvCommand( s:py_cmd . 'swank.' . a:cmd . '()' )
            call SlimvRefreshReplBuffer()
            if s:sldb_level < 0
...
function! SlimvDebugRestartFrame()
    ...
    if frame != ''
        call SlimvCommand( s:py_cmd . 'swank.swank_restart_frame("' . frame . '")' )
        call SlimvRefreshReplBuffer()
    endif
endfunction
...
function! SlimvListThreads()
    if SlimvConnectSwank()
        call SlimvCommand( s:py_cmd . 'swank.swank_list_threads()' )
        call SlimvRefreshReplBuffer()
    endif
endfunction
...
function! SlimvKillThread() range
    if SlimvConnectSwank()
        if a:firstline == a:lastline
            ...
            if item != ''
                call SlimvCommand( s:py_cmd . 'swank.swank_kill_thread(' . item . ')' )
                call SlimvRefreshReplBuffer()
            endif
...
function! SlimvDebugThread()
    if SlimvConnectSwank()
        ...
        if item != ''
            call SlimvCommand( s:py_cmd . 'swank.swank_debug_thread(' . item . ')' )
            call SlimvRefreshReplBuffer()
        endif
    endif
endfunction
...
function! SlimvArglist( ... )
    ...
    if s:swank_connected && !s:read_string_mode && c > 0 && line[c-1] =~ '\k\|)\|\]\|}\|"'
        ...
        if arg != ''
            ...
            call SlimvFindPackage()
            let msg = SlimvCommandGetResponse( ':operator-arglist', s:py_cmd . 'swank.swank_op_arglist("' . arg . '")', 0 )
            if msg != ''
...
function! SlimvConnectServer()
    if s:swank_connected
        execute s:py_cmd . "swank.swank_disconnect()"
...
function! SlimvInteractiveEval()
    ...
    if frame != ''
        ...
        if e != ''
            let result = SlimvCommandGetResponse( ':eval-string-in-frame', s:py_cmd . 'swank.swank_eval_in_frame("' . e . '", ' . frame . ')', 0 )
            if result != ''
                redraw
                echo result
            endif
...
function! SlimvUndefineFunction()
    if s:swank_connected
        call SlimvCommand( s:py_cmd . 'swank.swank_undefine_function("' . SlimvSelectSymbol() . '")' )
        call SlimvRefreshReplBuffer()
    endif
endfunction
...
function! SlimvMacroexpand()
    if SlimvConnectSwank()
        ...
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_macroexpand("s:swank_form")' )
    endif
endfunction
...
function! SlimvMacroexpandAll()
    if SlimvConnectSwank()
        ...
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_macroexpand_all("s:swank_form")' )
    endif
endfunction
...
function! SlimvBreakOnException()
    if SlimvGetFiletype() =~ '.*clojure.*' && s:SinceVersion( '2010-11-13' )
        ...
        if SlimvConnectSwank()
            ...
            call SlimvCommand( s:py_cmd . 'swank.swank_break_on_exception(' . s:break_on_exception . ')' )
            call SlimvRefreshReplBuffer()
...
function! SlimvBreak()
    if SlimvConnectSwank()
        ...
        if s != ''
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_set_break("' . s . '")' )
            redraw!
        endif
    endif
endfunction
...
function! SlimvTrace()
    ...
    if SlimvConnectSwank()
        ...
        if s != ''
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_toggle_trace("' . s . '")' )
            redraw!
        endif
    endif
endfunction
...
function! SlimvUntrace()
    ...
    if SlimvConnectSwank()
        let s:refresh_disabled = 1
        call SlimvCommand( s:py_cmd . 'swank.swank_untrace_all()' )
        let s:refresh_disabled = 0
        call SlimvRefreshReplBuffer()
    endif
endfunction
...
function! SlimvDisassemble()
    ...
    if SlimvConnectSwank()
        ...
        if s != ''
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_disassemble("' . s . '")' )
        endif
    endif
endfunction
...
function! SlimvInspect()
    ...
    if frame != ''
        ...
        if s != ''
            ...
            call SlimvCommand( s:py_cmd . 'swank.swank_inspect_in_frame("' . s . '", ' . frame . ')' )
            call SlimvRefreshReplBuffer()
        endif
    else
        ...
        if s != ''
            let s:inspect_path = [s]
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_inspect("' . s . '")' )
        endif
    endif
endfunction
...
function! SlimvXrefBase( text, cmd )
    if SlimvConnectSwank()
        ...
        if s != ''
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_xref("' . s . '", "' . a:cmd . '")' )
        endif
    endif
endfunction
...
function! SlimvProfile()
    if SlimvConnectSwank()
        ...
        if s != ''
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_toggle_profile("' . s . '")' )
            redraw!
        endif
    endif
endfunction
...
function! SlimvProfileSubstring()
    if SlimvConnectSwank()
        ...
        if s != ''
            ...
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_profile_substring("' . s . '","' . p . '")' )
            redraw!
        endif
    endif
endfunction
...
function! SlimvUnprofileAll()
    if SlimvConnectSwank()
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_unprofile_all()' )
    endif
endfunction
...
function! SlimvShowProfiled()
    if SlimvConnectSwank()
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_profiled_functions()' )
    endif
endfunction
...
function! SlimvProfileReport()
    if SlimvConnectSwank()
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_profile_report()' )
    endif
endfunction
...
function! SlimvProfileReset()
    if SlimvConnectSwank()
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_profile_reset()' )
    endif
endfunction
...
function! SlimvCompileDefun()
    ...
    if SlimvConnectSwank()
        let s:swank_form = SlimvGetSelection()
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_compile_string("s:swank_form")' )
    endif
    call winrestview( oldpos )
endfunction
...
function! SlimvCompileLoadFile()
    ...
    if SlimvConnectSwank()
        let s:compiled_file = ''
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_compile_file("' . filename . '")' )
        let starttime = localtime()
        ...
        if s:compiled_file != ''
            ...
            call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_load_file("' . s:compiled_file . '")' )
            let s:compiled_file = ''
        endif
    endif
endfunction
...
function! SlimvDescribeSymbol()
    if SlimvConnectSwank()
        ...
        call SlimvCommandUsePackage( s:py_cmd . 'swank.swank_describe_symbol("' . symbol . '")' )
    endif
endfunction
...
function! SlimvDescribe(arg)
    ...
    call SlimvFindPackage()
    let arglist = SlimvCommandGetResponse( ':operator-arglist', s:py_cmd . 'swank.swank_op_arglist("' . arg . '")', 0 )
    if arglist == ''
        " Not able to fetch arglist, assuming function is not defined
        " Skip calling describe, otherwise SWANK goes into the debugger
        return ''
    endif
    let msg = SlimvCommandGetResponse( ':describe-function', s:py_cmd . 'swank.swank_describe_function("' . arg . '")', 0 )
    if msg == ''
...
function! SlimvComplete( base )
    ...
    if s:swank_connected && !s:read_string_mode
        ...
        if g:slimv_simple_compl
            let msg = SlimvCommandGetResponse( ':simple-completions', s:py_cmd . 'swank.swank_completions("' . a:base . '")', 0 )
        else
            let msg = SlimvCommandGetResponse( ':fuzzy-completions', s:py_cmd . 'swank.swank_fuzzy_completions("' . a:base . '")', 0 )
        endif
...
function! SlimvFindDefinitionsForEmacs( symbol )
    if g:slimv_tags_file == ''
        ...
        let msg = SlimvCommandGetResponse( ':find-definitions-for-emacs', s:py_cmd . 'swank.swank_find_definitions_for_emacs("' . a:symbol . '")', 0 )
    endif
    try
        if msg != ''
            exec ":tjump " . msg
...
function! SlimvSetPackage()
    if SlimvConnectSwank()
        ...
        if pkg != ''
            let s:refresh_disabled = 1
            call SlimvCommand( s:py_cmd . 'swank.swank_set_package("' . pkg . '")' )
            let s:refresh_disabled = 0
            call SlimvRefreshReplBuffer()
        endif
    endif
...
function! SlimvQuitRepl()
    if s:swank_connected
        call SlimvCommand( s:py_cmd . 'swank.swank_quit_lisp()' )
        let s:swank_connected = 0
